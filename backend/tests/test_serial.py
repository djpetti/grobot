...
class SerialTalkerTestBase(base_test.BaseTest):
  """ A core base class that initializes a SerialTalker with a fake TTY. """
  ...
  def setUp(self):
    ...
    self._serial_talker = serial_talker.SerialTalker(115200,
                                                     device=device_name,
                                                     ioloop=self.io_loop)
  ...
  def tearDown(self):
    ...
    try:
      ...
      # The IOLoop may have cleaned it up for us.
      pass

    self._serial_talker.cleanup()

  def _wait_for_serial(self, expected):
    """ Wait to receive from the serial, and compares it with an expected value.
    Args:
      expected: The expected value. """
    data = os.read(self._conn, len(expected))
    self.assertEqual(expected, data.decode("utf8"))

class SerialTalkerTest(SerialTalkerTestBase):
  """ Tests for the SerialTalker class. """

  def test_simple_message(self):
    """ Tests sending a message with no fields. """
    self._serial_talker.write_command("TEST", 2)

    self._wait_for_serial("<TEST/12>")

  def test_message_with_fields(self):
    """ Tests sending a message with fields. """
    self._serial_talker.write_command("TEST", 2, 42, None, "hello")

    self._wait_for_serial("<TEST/12/42/None/hello>")
  ...
  def test_simple_message_read(self):
    ...
    self._serial_talker.add_message_handler(receive_message)

    os.write(self._conn, "<TEST/12>".encode("utf8"))
  ...
  def test_multiple_callbacks(self):
    ...
    self._serial_talker.add_message_handler(receive_message1)
    self._serial_talker.add_message_handler(receive_message2)
    ...
    self._serial_talker.remove_message_handler(receive_message1)
    self._serial_talker.remove_message_handler(receive_message2)
    # However, it should not let us remove them again.
    self.assertRaises(KeyError,
                      self._serial_talker.remove_message_handler,
                      receive_message1)
  ...
  def test_message_read_with_fields(self):
    ...
    os.write(self._conn, "<TEST/12/42/None/hello>".encode("utf8"))
  ...
  def test_read_partial_message(self):
    ...
    os.write(self._conn, "<TEST/12/1><TEST/".encode("utf8"))
    self.wait()
    # Add the rest of the message.
    os.write(self._conn, "12/2>".encode("utf8"))
    self.wait()
